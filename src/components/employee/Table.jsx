    ...
    <div className="overflow-x-auto border rounded-lg shadow-md ">
      <table className="w-full text-left border-collapse border-[#DFDFDF]">
        ...
        <tbody>
          {data.length > 0 ? (
            data.map((row, idx) => (
             <tr key={idx} className="bg-transparent font-semibold hover:bg-gray-50">
  {columns.map((col) => (
    <td
      key={col.key}
      ...
      {col.key === "status" ? (
        <span
          className={`px-2 py-1 rounded-full  text-sm ${
            row[col.key] === "active" ? " border-1 border-green-700 text-green-700 bg-[#b8f0d1]" : "bg-red-200 border-1 border-red-700 text-red-700"
          }`}
        >
          {row[col.key]}
        </span>
